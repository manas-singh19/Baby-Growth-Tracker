...
const DEFAULT_PROFILE: BabyProfile = {
  ...
  name: 'Baby',
  birthDate:
    new Date(Date.now() - 180 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] + 'T00:00:00.000Z', // 6 months ago
  gender: 'female',
};
...
export async function saveMeasurement(
  measurement: Omit<
    GrowthMeasurement,
    'ageInDays' | 'weightPercentile' | 'heightPercentile' | 'headPercentile'
  >,
  profile: BabyProfile
  ...
  const headPercentile = calculatePercentile(measurement.headCm, ageInDays, 'head', profile.gender);
